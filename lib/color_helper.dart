...
Future<Uint8List?> get_image_data(String url) async {
...
Future<Color> extractColor(String url) async {
  int noOfPaletteColors = Random().nextInt(4) + 2;
  ...
  List<Color> colors = await compute(extractPixelsColors, imageBytes);
  List<Color> sortedColors = await compute(sortColors, colors);
  List<Color> palette = await compute(
      generatePalette, {keyPalette: colors, keyNoOfItems: noOfPaletteColors});

  return getAverageColor(palette);
}
